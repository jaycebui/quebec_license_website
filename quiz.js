function checkScore() {
    ...
    // 1. RESET STYLES (Clean slate for re-submission)
    ...
    // 2. VALIDATION: Identify UNANSWERED questions
    ...
    // 3. SCORING & FEEDBACK: Highlight Green/Red and track WRONG answers
    for (let i = 1; i <= total; i++) {
        const qName = `q${i}`;
        const selected = form.querySelector(`input[name="${qName}"]:checked`);
        ...
        if (selected.value === correctValue) {
            ...
            wrongQuestions.push(i); // Track the number of the wrong answer
        }
